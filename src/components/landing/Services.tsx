...
export default function Services({ services, content }: ServicesProps) {
  ...
  return (
    <div id="services" className="bg-primary">
      <div className="py-20 md:py-[110px] px-6 sm:px-8 md:px-14 max-w-[1200px] mx-auto">
        <MotionReveal>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-primary-foreground/8 border border-primary-foreground/8 mt-12 md:mt-14">
  ...
  return (
    <MotionReveal delay={0.12 * (index + 1)}>
      ...
      >
        ...
        <div lang={isBn ? 'bn' : 'en'} className="font-heading text-[22px] md:text-xl font-normal text-primary-foreground mb-3.5 leading-[1.2] md:leading-tight">
          {isBn
            ? (index === 0 ? 'সোশ্যাল মিডিয়া ডিজাইন' : index === 1 ? 'প্রিমিয়াম বাংলা ভিজ্যুয়াল ডিজাইন' : service.name_bn || service.name_en)
            : service.name_en}
        </div>
        <p lang={isBn ? 'bn' : 'en'} style={isBn ? undefined : { fontFamily: "'DM Sans', sans-serif" }} className="text-[14px] md:text-[13px] leading-[1.7] md:leading-[1.75] text-primary-foreground/50 relative z-10">
